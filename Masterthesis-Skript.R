...
# =========================================================
# 1) PROM Scores berechnen inkl. Missing-Data-Regeln
# =========================================================

min_items_BRS   <- 4   # mindestens 4 von 6 Items
min_items_SIRSI <- 10  # mindestens 10 von 12 Items
min_items_WOSI  <- 17  # mindestens 17 von 21 Items
...
# =========================================================
# Schulterbelastung in 3 Gruppen
# =========================================================

RTS_final <- RTS_final %>%
  mutate(
    schulter3 = case_when(
      schulterbelastung %in% c(3, 5) ~ "Kein Überkopf / Zug",
      schulterbelastung == 2 ~ "Überkopf",
      schulterbelastung %in% c(0, 1) ~ "Kontakt",
      TRUE ~ NA_character_
    ),
    schulter3 = factor(
      schulter3,
...
# =========================================================
# 6) Primäre Regressionsmodelle OHNE Shoulder Load
# =========================================================
...
cat("Primary Model A3 complete cases n =", nrow(modelA3_df), "\n")
cat("Primary Model B3 complete cases n =", nrow(modelB3_df), "\n")

modelA3 <- lm(
  SIRSI_3m ~ BRS_baseline + delta_BRS,
  data = modelA3_df
)

modelB3 <- lm(
  WOSI_3m ~ BRS_baseline + delta_BRS,
  data = modelB3_df
)
...
# =========================================================
# 6.1) Adjustierte Modelle MIT Shoulder Load
# =========================================================

modelA3_adj_df <- modelA3_df %>%
  filter(!is.na(schulter3))

modelB3_adj_df <- modelB3_df %>%
  filter(!is.na(schulter3))

cat("Adjusted Model A3 complete cases n =", nrow(modelA3_adj_df), "\n")
cat("Adjusted Model B3 complete cases n =", nrow(modelB3_adj_df), "\n")
...
Anova(modelA3_adj, type = "II")
Anova(modelB3_adj, type = "II")

coeftest(modelA3_adj, vcov. = vcovHC(modelA3_adj, type = "HC3"))
coeftest(modelB3_adj, vcov. = vcovHC(modelB3_adj, type = "HC3"))
...
diagnose_lm(modelA3, modelA3_df, "Primary Model A3: SIRSI")
diagnose_lm(modelB3, modelB3_df, "Primary Model B3: WOSI")

diagnose_lm(modelA3_adj, modelA3_adj_df, "Adjusted Model A3: SIRSI + Shoulder Load")
diagnose_lm(modelB3_adj, modelB3_adj_df, "Adjusted Model B3: WOSI + Shoulder Load")
...
cat("\nEinflussreiche Fälle Primary Model A3:\n")
...
if(length(infl_A3) > 0){
  ...
  cat("\nOriginal Primary Model A3:\n")
  print(coef(summary(modelA3)))
  
  cat("\nSensitivity Primary Model A3:\n")
...
cat("\nEinflussreiche Fälle Primary Model B3:\n")
...
if(length(infl_B3) > 0){
  ...
  cat("\nOriginal Primary Model B3:\n")
  print(coef(summary(modelB3)))
  
  cat("\nSensitivity Primary Model B3:\n")
...
# =========================================================
# 9.1) Standardisierte Effekte
# =========================================================

modelA3_std <- lm(
  scale(SIRSI_3m) ~ scale(BRS_baseline) + scale(delta_BRS),
  data = modelA3_df
)
...
modelB3_std <- lm(
  scale(WOSI_3m) ~ scale(BRS_baseline) + scale(delta_BRS),
  data = modelB3_df
)
...
modelA3_adj_std <- lm(
  scale(SIRSI_3m) ~ scale(BRS_baseline) +
    scale(delta_BRS) +
    schulter3,
  data = modelA3_adj_df
)

summary(modelA3_adj_std)
...
coef_A3_adj <- extract_regression_table(modelA3_adj)
coef_B3_adj <- extract_regression_table(modelB3_adj)

info_A3 <- extract_model_info(modelA3, "Primary Model A3: SIRSI at 3 months")
info_B3 <- extract_model_info(modelB3, "Primary Model B3: WOSI at 3 months")

info_A3_adj <- extract_model_info(modelA3_adj, "Adjusted Model A3: SIRSI + shoulder load")
info_B3_adj <- extract_model_info(modelB3_adj, "Adjusted Model B3: WOSI + shoulder load")
...
ft_info_A3_adj <- flextable(info_A3_adj) %>%
  autofit() %>%
  theme_box()

ft_coef_A3_adj <- flextable(coef_A3_adj) %>%
  set_header_labels(
    Predictor = "Predictor",
    B = "B",
    CI_95 = "95% CI",
...
ft_info_B3_adj <- flextable(info_B3_adj) %>%
  autofit() %>%
  theme_box()

ft_coef_B3_adj <- flextable(coef_B3_adj) %>%
  set_header_labels(
    Predictor = "Predictor",
    B = "B",
    CI_95 = "95% CI",
...
doc <- body_add_par(doc, "Primary Model A3: SIRSI at 3 months", style = "heading 2")
...
doc <- body_add_par(doc, "Primary Model B3: WOSI at 3 months", style = "heading 2")
doc <- body_add_flextable(doc, ft_info_B3)
doc <- body_add_par(doc, "Regression coefficients", style = "heading 3")
doc <- body_add_flextable(doc, ft_coef_B3)

doc <- body_add_par(doc, "Adjusted Model A3: SIRSI + shoulder load", style = "heading 2")
doc <- body_add_flextable(doc, ft_info_A3_adj)
doc <- body_add_par(doc, "Regression coefficients", style = "heading 3")
doc <- body_add_flextable(doc, ft_coef_A3_adj)

doc <- body_add_par(doc, "Adjusted Model B3: WOSI + shoulder load", style = "heading 2")
doc <- body_add_flextable(doc, ft_info_B3_adj)
doc <- body_add_par(doc, "Regression coefficients", style = "heading 3")
doc <- body_add_flextable(doc, ft_coef_B3_adj)

doc <- body_add_par(
  doc,
  "Primary models included BRS baseline and ΔBRS as predictors. Adjusted models additionally included sport-specific shoulder load.",
  style = "Normal"
)
